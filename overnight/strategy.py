...
class ClosingParams:
    ...
    ma_mid: int = 20
    # 강한 마감(양봉) 조건
    #   ↓ 기본값 = 실데이터(KOSPI 199종목·2019~2026) 워크포워드 최적화에서
    #     검증(미래)구간 기대값이 가장 컸던 강건 조합.
    up_min: float = 0.03          # 당일 상승률(전일종가 대비) ≥ +3%
    close_pos_min: float = 0.50   # 종가가 당일 (고-저) 범위 상단 50% 이상에서 마감
    # 거래량
    vol_ma: int = 20
    vol_mult: float = 2.0         # 당일 거래량 ≥ 20일 평균 × 2.0
    # 돌파
    breakout_lookback: int = 60   # 최근 N일 고가 대비 돌파 여부
    # 과열 필터
    rsi_period: int = 14
    rsi_high: float = 85.0        # RSI 이 값 이상이면 과열로 보고 제외
