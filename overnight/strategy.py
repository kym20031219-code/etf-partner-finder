...
class ClosingParams:
    ...
    ma_mid: int = 20
    # 강한 마감(양봉) 조건
    #   ↓ 기본값 = 실데이터(KOSPI 149종목·2019~2026) '승률 최적화 + 국면필터'
    #     워크포워드 검증에서 뽑은 강건 조합. (국면필터는 스캔 시 --regime 로 적용)
    #     검증(미래)구간: 승률 49.6% · 건당 +1.01% · 손익비 2.08 · MDD -12.6%.
    up_min: float = 0.05          # 당일 상승률(전일종가 대비) ≥ +5%
    close_pos_min: float = 0.50   # 종가가 당일 (고-저) 범위 상단 50% 이상에서 마감
    # 거래량
    vol_ma: int = 20
    vol_mult: float = 3.0         # 당일 거래량 ≥ 20일 평균 × 3.0
    # 돌파
    breakout_lookback: int = 60   # 최근 N일 고가 대비 돌파 여부
    # 과열 필터
    rsi_period: int = 14
    rsi_high: float = 90.0        # RSI 이 값 이상이면 과열로 보고 제외
